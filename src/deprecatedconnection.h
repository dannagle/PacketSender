 ...
 *
 */
class DeprecatedConnection : public QObject
{
    Q_OBJECT

public:
    // target constructor
    explicit DeprecatedConnection(std::unique_ptr<TCPThread> thread,
                        bool isIncoming = false,
                        bool isSecure = false,
                        bool isPersistent = true,
                        qintptr socketDescriptor = -1,
                        QObject *parent = nullptr);
    explicit DeprecatedConnection(const QString &host,
                        quint16 port,
                        const Packet &initialPacket = Packet(),
                        QObject *parent = nullptr,
                        std::unique_ptr<TCPThread> thread = nullptr);
    // Server/incoming constructor
    explicit DeprecatedConnection(int socketDescriptor,
                        bool isSecure = false,
                        bool isPersistent = true,
                        QObject *parent = nullptr,
                        std::unique_ptr<TCPThread> thread = nullptr);
    ~DeprecatedConnection() override;
