...
bool PacketNetwork::hasJoinedMulticast(QUdpSocket *udp)
{
...
void PacketNetwork::joinMulticast(QUdpSocket * udp, QString address)
{
...
bool PacketNetwork::canSendMulticast(QString address)
{
    ...
    QUdpSocket *udp;
    foreach (udp, keys) {
        if(joinedMulticast[udp] == address) {
            return true;
        }
    }
    return false;
